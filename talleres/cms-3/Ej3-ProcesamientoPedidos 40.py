...
def procesamiento_pedidos(pedidos: Queue,
  ...
  # Solo toma los pedidos no vacios
...
def procesar_pedido(pedido: Queue,
  ...
  # Defino los elementos adicionales del pedido procesado 
  total_pedido = 0.0
  estado = 'completo'
  
  # Itero entre productos del pedido para ver si hay o no stock
  for producto, cantidad in productos.items():                                    
    if producto in stock_productos and stock_productos[producto] >= cantidad:   # Verifico que haya suficiente stock de tal producto
      stock_productos[producto] -= cantidad    # Resto la cantidad de productos que saque del stock
      total_pedido += precios_productos[producto] * cantidad   # Sumo al precio total 
    else:
      estado = 'incompleto'
      if producto in stock_productos:  # No hay else porque siempre existe el producto, simplemente el stock es 0
        ...
        total_pedido += precios_productos[producto] * cantidad_disponible

  # Defino al pedido procesado como un diccionario por cliente
  pedido_procesado = {'id': id_pedido, 'cliente': cliente, 'productos': productos, 'precio_total': total_pedido, 'estado': estado}
